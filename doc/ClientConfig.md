...
注意： ECDHE基于SM2密钥交换实现（关于SM2密钥交换，详见**GB/T 35276-2017**和**GB/T 32918.3-2016**），类似服务端，ECDHE密码套件需要客户端同时配置**认证密钥**和**加密密钥**才会启用。

可以通过下面方式手动指定握手使用的密码套件（绝大部分情况下不需要，除非你想限制某些密码套件的使用），这里的顺序不重要（密码套件的优先顺序由本实现固定）：
...
如果你想测试ECDHE_SM4_GCM_SM3/ECDHE_SM4_CBC_SM3，在客户端的CipherSuites配置中不能出现ECC_SM4_GCM_SM3、ECC_SM4_CBC_SM3，除非服务端只支持ECDHE密码套件。
