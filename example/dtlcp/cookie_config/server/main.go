...
// 服务端 DTLCP 特有参数配置示例
// 演示 Cookie、PMTU、重放窗口、重传超时等 DTLCP 特有参数的配置
func main() {
	config := &dtlcp.Config{
		Certificates: []dtlcp.Certificate{sigCert, encCert},
		// PMTU 路径 MTU，影响握手消息分片阈值
		PMTU: 1400,
		// CookieSecret 服务端用于生成和验证 HelloVerifyRequest Cookie 的密钥
		CookieSecret: []byte("my-32-byte-random-secret-key!!"),
		// ReplayWindow 重放攻击检测的滑动窗口大小
		ReplayWindow: 64,
		// InitialRetransmitTimeout 握手消息初始重传超时
		InitialRetransmitTimeout: 1 * time.Second,
		// MaxRetransmitTimeout 握手消息最大重传超时
		MaxRetransmitTimeout: 64 * time.Second,
	}
	ln, err := dtlcp.Listen("udp", ":8454", config)
	if err != nil {
		panic(err)
	}
	defer ln.Close()
