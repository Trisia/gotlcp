...
const (
	// maxPlaintext TLS/DTLS 记录层最大明文载荷，单位：字节。
...
const (
	...
	typeHelloVerifyRequest uint8 = 3 // DTLCP 新增：HelloVerifyRequest 消息
...
// uint24 表示 DTLCP 中的 24 位长度字段
type uint24 uint32

// uint48 表示 DTLCP 中的 48 位序列号
type uint48 uint64
...
var errNoCertificates = errors.New("dtlcp: no certificates configured")
...
const (
	statePreparing handshakeState = iota // 准备中：构造待发消息
	stateSending                         // 发送中：传输飞行消息
	stateWaiting                         // 等待中：等待对端响应
...
func defaultNewTimer(d time.Duration) *TimerHandle {
	t := time.NewTimer(d)
	return &TimerHandle{
		C:     t.C,
		Stop:  t.Stop,
