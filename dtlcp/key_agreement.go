...
var errClientKeyExchange = errors.New("dtlcp: invalid ClientKeyExchange message")
var errServerKeyExchange = errors.New("dtlcp: invalid ServerKeyExchange message")
...
func (e *eccKeyAgreement) generateServerKeyExchange(hs *serverHandshakeState) (*serverKeyExchangeMsg, error) {
	...
	if sigCert == nil && encCert == nil {
		return nil, errors.New("dtlcp: ecc key exchange need 2 certificates")
	}
	config := hs.c.config
	/*
	...
	*/
	...
	if !ok {
		return nil, errors.New("dtlcp: certificate private key does not implement crypto.Signer")
	}
...
func (e *eccKeyAgreement) processClientKeyExchange(hs *serverHandshakeState, ckx *clientKeyExchangeMsg) ([]byte, error) {
	...
	if cipher[0] != 0x30 {
		return nil, errors.New("dtlcp: bad client key exchange ciphertext format")
	}
	...
	if !ok {
		return nil, errors.New("dtlcp: certificate private key does not implement crypto.Decrypter")
	}
...
func (e *eccKeyAgreement) processServerKeyExchange(hs *clientHandshakeState, skx *serverKeyExchangeMsg) error {
	if len(hs.peerCertificates) < 2 {
		return errors.New("dtlcp: ecc key exchange need 2 certificates")
	}
	...
	if !ok {
		return errors.New("dtlcp: sm2 signing requires a sm2 public key")
	}
	...
	if !sm2.VerifyASN1WithSM2(pub, nil, tbs, sig) {
		return errors.New("dtlcp: processServerKeyExchange: sm2 verification failure")
	}
	return nil
}
...
func (e *eccKeyAgreement) generateClientKeyExchange(hs *clientHandshakeState) ([]byte, *clientKeyExchangeMsg, error) {
	if len(hs.peerCertificates) < 2 {
		return nil, nil, errors.New("dtlcp: ecc key exchange need 2 certificates")
	}
	encCert := hs.peerCertificates[1]
	config := hs.c.config
...
func (ka *sm2ECDHEKeyAgreement) generateServerKeyExchange(hs *serverHandshakeState) (*serverKeyExchangeMsg, error) {
	if hs.sigCert == nil || hs.encCert == nil {
		return nil, errors.New("dtlcp: ECDHE key exchange needs 2 certificates")
	}
	config := hs.c.config
	sigkey := hs.sigCert
	...
	default:
		return nil, fmt.Errorf("dtlcp: private key not support sm2 key exchange")
	}
...
func (ka *sm2ECDHEKeyAgreement) processClientKeyExchange(hs *serverHandshakeState, ckx *clientKeyExchangeMsg) ([]byte, error) {
	if len(hs.peerCertificates) < 2 {
		return nil, errors.New("dtlcp: sm2 key exchange need client enc cert")
	}
	responsePubKey, ok := hs.peerCertificates[1].PublicKey.(*ecdsa.PublicKey)
	if !ok {
		return nil, errors.New("dtlcp: client key not sm2 type")
	}
...
func (ka *sm2ECDHEKeyAgreement) processServerKeyExchange(hs *clientHandshakeState, skx *serverKeyExchangeMsg) error {
	if len(hs.peerCertificates) < 2 {
		return errors.New("dtlcp: sm2 key exchange need server provide two certificate")
	}
...
func (ka *sm2ECDHEKeyAgreement) generateClientKeyExchange(hs *clientHandshakeState) ([]byte, *clientKeyExchangeMsg, error) {
	...
	default:
		return nil, nil, fmt.Errorf("dtlcp: private key not support sm2 key exchange")
	}
	...
	if !ok {
		return nil, nil, errors.New("dtlcp: server encrypt certificate key type not sm2")
	}
